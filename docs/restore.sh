...
for i in users hiera-data nodes; do
  curl -s -k --cacert $CA -X POST $URI/$i -H "Content-Type: application/json" -H "Authorization: Bearer $TOKEN" --data-binary @$RESTORE_WORKING_DIR/$i.json > $BACKUP_WORKING_DIR/$i.json
done
